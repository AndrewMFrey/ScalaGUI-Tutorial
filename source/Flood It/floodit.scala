// -------------------------------------------------------------------------------------
// Flood-It! Game (based off tutorial found at http://otfried.org/scala/floodit.html)
// -------------------------------------------------------------------------------------
...
class Blocks(val width: Int, val height: Int) {
  ...
  def set(pos: Pos, color: Int): Unit = {
...
class Canvas(val blocks: Blocks) extends Component {
  ...
  override def paintComponent(g: Graphics2D): Unit = {
    ...
    val colWid = d.height / blocks.width
...
class FloodUI extends MainFrame {
  ...
  def newGame(): Unit = {
    if (Dialog.showConfirmation(canvas, "Do you want to start a new game?", title = FloodIt.Title) == Dialog.Result.Ok)
      restartGame()
  }
...
object FloodIt {
  ...
  def colorFor(n: Int): Color = {
    n match {
      ...
      case 4 => Color.pink
      case 5 => Color.cyan
      case _ => Color.white // should never happen
    }
  }
